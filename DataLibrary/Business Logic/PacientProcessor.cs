...
namespace DataLibrary.Business_Logic
{
    public static class PacientProcessor
    {
        ...
        public static List<PacientModel> LoadSpecificPacient(string email)
        {
            string sql = @"select dbo.Login.email, dbo.Pacient.nume, dbo.Pacient.prenume, dbo.Pacient.cnp, dbo.Pacient.adresa, dbo.Pacient.telefon, dbo.Pacient.varsta, dbo.Pacient.inaltime, dbo.Pacient.greutate from dbo.Pacient inner join dbo.Login on dbo.Pacient.email = dbo.Login.email where dbo.Login.email = " + email + ";";
            return SQLDataAceess.LoadData<PacientModel>(sql);
        }
    }
}
